###____Utility-Geo____ ----
# Miscellaneous geo functions for censusprofiler.
...
build_map <- function(mapDF=NULL,
  ...
  if(radiusOnly==FALSE & areaOnly==FALSE){
    ...
    value <- "name_clean"
    pal <- paletteer::paletteer_d("ggpomological::pomological_palette")
  }
  ...
  map <- tmap::tm_basemap(server=providers$Stadia.StamenTonerLite) +
    ...
    {if(i==FALSE){
      {if(dispRoads==TRUE && geo_resolution>=1){
        {if(nrow(roadlines)>0){
          tmap::tm_shape(roadlines) +
            tmap::tm_lines(col="#6F6F6F") }}}} +
        {if(dispRails==TRUE && geo_resolution>=1){
          {if(nrow(rails)>0){
            tmap::tm_shape(rails) +
              tmap::tm_lines(col="darkblue") }}}} +
        {if(dispRoads==TRUE && geo_resolution>=1){
          {if(nrow(roadnames)>0){
            tmap::tm_shape(roadnames) +
              tmap::tm_text("FULLNAME",size=1/2,remove.overlap = TRUE) }}}} +
...
#' @param density Logical parameter to display cloropleth instead of dots.
#' @param density_col Name of density column in addressList. 
#' @param verbose Logical param to provide feedback.
#'
...
map_locations <- function(addressList,
  ...
  if(density==TRUE){
    if(is.null(density_col)){
      ...
      geo_states <- dplyr::left_join(geo_states,dens,join_by(STATEFP==state_density))
    }else{
